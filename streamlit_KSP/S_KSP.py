...
st.caption("클릭 중심 지도 + 프로 테마 + 글로벌 대시보드 + 상대 트렌드 분석 (Plotly 강화판)")
...
STOP = {
    ...
    "강화와", "현행", "생태계를", "현황과", "가치평가", "전환에", "활동", "국민", "포함", "기본계획", "접근성", "전환", "할당", "양국", "효과", "추진", "협력과", "이용계획", "담은", "로드맵을", "제시하였다", "WASH", "기술", "기대효과", "가나의", "솔루션을"
}
...
if mode == "국가별 총계":
    ...
    if clicked_iso:
        ...
        if not sub.empty:
            ...
            st.markdown(f"### {country_name} — 보고서 {sub['파일명'].nunique()}건")
...
# ===================== ② WB Class 단일클래스 (지도를 국가 하이라이트로만 사용, 상세는 '클래스 전체' 기준) =====================
    ...
    st.subheader("WB Class 단일클래스 보고서 수")
    ...
    st.markdown(f"### {sel} — 전체 보고서 {n_docs:,}건 · 참여국가 {part_countries:,}개국")
    ...
    with tab_overview:
        ...
        with cB:
            st.metric("보고서 수", f"{n_docs:,}")
        with cC:
            st.metric("대상기관 수", f"{sub_wb['대상기관'].nunique():,}")

        # 참여국가 Top-10 (보고서 수 기준)
        st.markdown("#### 참여국가 (보고서 수 Top 10)")
